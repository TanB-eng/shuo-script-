...
// excludeIds：可选，单个 user_id 或数组/Set 列表；命中者不选（如放弃追击冷却中的目标），
// 这样能选中"次优"的另一个富人，而不是因为最佳目标被跳过就什么都不打。
export function chooseAggroTarget(state, excludeIds = null) {
  if (!state.self) return null;

  const excluded = new Set();
  if (excludeIds != null) {
    for (const v of (Array.isArray(excludeIds) ? excludeIds : [excludeIds])) {
      if (v != null) excluded.add(Number(v));
    }
  }
  ...
  for (const p of state.visiblePlayers()) {
    ...
    // 被排除的目标（放弃追击冷却中）不选
    if (excluded.has(Number(p.user_id))) continue;
