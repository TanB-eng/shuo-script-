...
const DEFAULTS = {
  ...
  //
  // 取 300 点(=3000m 机动余量)，而【不是】一次传送的 1500 点。理由：
  //   · 1h 额度只有 3000 点，而移动 10m/s 就耗 1 点/秒 —— 拾金 bot 一直在跑，
  //     若门槛设 1500(50%)，连续移动约 25 分钟后主动攻击就【静默停止】，
  //     实测表现为"10 金币的人靠到身边也毫无反应"。
  //   · "必须保住传送"这个理由不成立：掉金币的条件是【死亡】，下线并不掉金币。
  //     传不了就下线，代价只是 90s 时间，金币不会损失。
  //   · 真正必须防的是 1h 体力见底 —— 那会完全无法移动，变成活靶子。
  //     300 点足够跑 3000m 脱离。
  aggroStaminaReserveMillis: 300 * 1000,
